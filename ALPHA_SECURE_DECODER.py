...
# -*- coding: utf-8 -*-
"""
ALPHA SECURE DECODER - Strategic Code Decoding System
Enhanced with Mac compatibility and secure communication protocols
"""
import os
import sys
import time
...
class SecureAlphaDecoder:
    """Military-grade encrypted ALPHA code decoder"""
    ...
    def receive_secure_codes(self):
        """Receive and decrypt synchronized codes from server"""
        try:
                    ...
                    if decrypted_data:
                        try:
                            ...
                            if message['type'] == 'codes' and message.get('encrypted'):
                                ...
                                print(f"\n{COLOR_GREEN}{'='*80}")
                                print(f"{COLOR_GREEN}{'*** SECURE ALPHA CODE RECEIVED ***':^80}")
                                print(f"{COLOR_GREEN}{'='*80}")
                                print(f"{COLOR_WHITE}ALPHA CODE: {COLOR_GREEN}{message['alpha']}{COLOR_RESET}")
    ...
    def login(self):
        ...
        try:
            ...
            alpha_code = country_codes.get(choice, "UNKNOWN-0000")
        except (EOFError, KeyboardInterrupt):
            print(f"\n{COLOR_RED}Selection interrupted.")
            return False
        
        # Show ALPHA ASCII Art Box
        clear_screen()
        print(COLOR_RED)
        print(f"{'╔═════════════════════════════════════════════════╗':^80}")
        print(f"{'║   █████╗   ██╗      ██████╗  ██╗  ██╗  █████╗   ║':^80}")
        ...
        # Display ALPHA CODE in YELLOW background with BLACK text
        COLOR_BLACK = "\033[30m"
        print(f"\n{BG_YELLOW}{COLOR_BLACK}{f'ALPHA CODE: {alpha_code}':^80}{COLOR_RESET}")
...
def main():
    ...
    print(f"\n{BG_RED}{COLOR_WHITE}{'ALPHA PHASE INITIATED':^80}{COLOR_RESET}")
    time.sleep(1)
    ...
    # Start enhanced keepalive thread immediately (before login)
    import threading
    def keepalive():
        count = 0
        while True:
            try:
                if decoder.client_socket:
                    ...
                    if count <= 3:  # Show only first 3 messages
                        print(f"{COLOR_DIM}ALPHA Keepalive sent to WOLF_OS (#{count}){COLOR_RESET}")
                time.sleep(3)
            except:
                break
    ...
    print(f"{COLOR_GREEN}✓ ALPHA Keepalive thread started - Connection will stay active")
    ...
    try:
        ...
        if data:
            try:
                ...
                if msg.get('type') == 'handshake':
                    ...
                    response = json.dumps({'decoder_type': 'ALPHA'})
                    decoder.client_socket.send(response.encode())
                    print(f"{COLOR_GREEN}✓ Sent ALPHA identification to WOLF_OS: {response}")
        ...
        try:
            response = json.dumps({'decoder_type': 'ALPHA'})
            decoder.client_socket.send(response.encode())
            print(f"{COLOR_GREEN}✓ Sent ALPHA identification to WOLF_OS (fallback): {response}")
    ...
    print(f"{COLOR_GREEN}✓ ALPHA decoder now maintaining connection for WOLF_OS...")
    ...
    try:
        while True:
            try:
                ...
                if data:
                    try:
                        ...
                        if msg.get('type') == 'codes':
                            print(f"{COLOR_GREEN}✓ ALPHA received codes: {msg.get('alpha', 'N/A')}")
                            # Send acknowledgment
                            decoder.client_socket.send(b'ACK')
                        elif msg.get('type') == 'shutdown':
                            print(f"{COLOR_YELLOW}✓ ALPHA received shutdown signal from WOLF_OS")
                            break
                    except:
                        pass
                else:
                ...
                print(f"{COLOR_RED}✗ ALPHA connection error: {e}")
                break
    except KeyboardInterrupt:
        print(f"\n{COLOR_YELLOW}✓ ALPHA decoder shutting down...")
    
    print(f"{COLOR_GREEN}✓ ALPHA decoder connection closed")
